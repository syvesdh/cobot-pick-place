...
bool TmDriver::run_pvt_traj(const TmPvtTraj &pvts)
{
	...
	// Custom Timeout Block: Wait up to 2000ms for Listen Node to reconnect
	if (!sct.is_connected()) {
		print_info("TM_DRV: Listen node not connected. Awaiting connection (up to 2000ms)...");
		auto wait_start = std::chrono::steady_clock::now();
		while (!sct.is_connected()) {
			std::this_thread::sleep_for(std::chrono::milliseconds(10));
			if (std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - wait_start).count() > 2000) {
				print_error("TM_DRV: Timed out waiting for SCT connection. Dropping trajectory.");
				return false;
			}
		}
		print_info("TM_DRV: Listen node reconnected. Proceeding with trajectory.");
	}
